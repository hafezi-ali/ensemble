...
\greenhighlight{In summary, while existing dynamic ensemble methods like K-means-based (e.g., DELAK) and meta-learning based (e.g., META-DES.Oracle) have advanced the field, they often face limitations in handling highly complex and overlapping data distributions, potential computational overhead, or sensitivity to the geometric definition of competence regions. Our proposed DDEL-GMM differentiates itself by integrating Gaussian Mixture Models for sophisticated data clustering—capturing multi-modal and non-spherical data patterns—and a subsequent density-based dynamic selection strategy. This approach directly addresses the challenges of achieving robust personalization and adaptability in complex, noisy environments. The primary contribution of DDEL-GMM to the literature is thus a more flexible and statistically grounded framework for dynamic ensemble learning, designed to improve predictive accuracy and decision-making efficiency by more accurately modeling data sub-structures and employing a nuanced, density-informed mechanism for classifier selection, especially for data exhibiting intricate structures, as often encountered in real-world applications like sensor-based activity recognition.}
	...
	\end{figure}
		...
		\noindent \textbf{Result.} We compared DDEL-GMM against five non-clustering baselines on the HAR dataset under the identical $10$-fold protocol: a single logistic-regression classifier, bagging with logistic-regression base estimators \cite{breiman_bagging_1996}, random forests \cite{breiman_random_2001}, AdaBoost \cite{freund_decision-theoretic_1997}, and gradient boosting \cite{friedman_greedy_2001}. All baselines used the same $157$-principal-component representation, so the comparison isolates the ensemble construction strategy rather than the feature space. Table~\ref{tab:noclustering} reports the outcome. DDEL-GMM attains the highest mean F-score ($0.968 \pm 0.003$) and the best mean rank ($1.1$), ahead of the strongest classic ensemble, random forests ($0.962 \pm 0.006$). Every pairwise comparison favours DDEL-GMM under the one-sided Wilcoxon signed-rank test ($p \leq 0.002$, $n = 10$ folds).
		
		\noindent \textbf{The strongest baseline is not separated by the omnibus test.} Pairwise tests across six methods invite an inflated false-positive rate, so we also applied a Friedman omnibus test followed by a Nemenyi post-hoc procedure. The omnibus test confirms that the six methods are not equivalent ($\chi^2 = 39.83$, $\mathrm{df} = 5$, $p = 1.6 \times 10^{-7}$). The post-hoc procedure, however, is more conservative: at $k = 6$ methods and $N = 10$ folds the critical difference is $2.38$ mean ranks, and the gap between DDEL-GMM and random forests is $1.30$. We therefore report plainly that \emph{DDEL-GMM is not distinguishable from random forests under the omnibus correction}, notwithstanding the significant pairwise test. The separation from gradient boosting, bagging, AdaBoost, and the single logistic-regression model does exceed the critical difference. The honest reading is that DDEL-GMM is reliably better than four of the five classic ensembles and competitive with, but not statistically separated from, the strongest of them on this dataset at this number of folds.
		
		\noindent \textbf{Does the weighting scheme earn its place?} The comparison above bundles two ingredients---partitioning the space and weighting the resulting classifiers by density-based competence. The ablation of Section~\ref{subsec:gmm_vs_kmeans} separates them. Replacing density-based weighting with the distance-based selection of DELAK while retaining clustering costs $1.6$ points of F-score (Table~\ref{tab:sota_comparison}), and replacing GMM partitioning with K-means while retaining density weighting costs $0.7$ points (Table~\ref{tab:clustering_ablation}). Both ingredients therefore contribute, and neither alone accounts for the margin over the non-clustering baselines. The ordering within Table~\ref{tab:noclustering} is consistent with this reading: the two tree ensembles that construct many diverse learners over the full space are the most competitive, while the single global model and bagging---which perturbs the training sample but not the region of competence---are weakest.
		
		\noindent \textbf{Cost.} DDEL-GMM trains $K$ base classifiers plus a Gaussian mixture, so it is not the cheapest method in the comparison; the single logistic-regression model ($6.8$\,s) and bagging ($21.3$\,s) are substantially faster. Its training time ($48.2$\,s) is comparable to random forests ($52.7$\,s) and below gradient boosting ($88.4$\,s), placing it at a favourable point on the accuracy--cost frontier of Figure~\ref{fig:noclustering}b rather than at the cheap end of it. Inference additionally requires evaluating $K$ Gaussian densities per test instance, a cost the flat ensembles do not incur.
		
		\noindent \textbf{Insight.} Partitioning the feature space into locally coherent regions and weighting specialized learners by local competence extracts structure that neither a single global model nor an unstructured committee captures, and the effect is large enough to separate our method from four of five classic ensembles on a dataset where all of them perform well. Against the strongest tree ensemble, the advantage is consistent in direction across all ten folds but within the critical difference of the omnibus test---so the defensible claim is competitiveness with random forests at comparable cost, together with a decomposition (Tables~\ref{tab:sota_comparison} and~\ref{tab:clustering_ablation}) showing that both the mixture-model partitioning and the density-based weighting contribute to it. A larger number of folds or additional datasets would be required to resolve the DDEL-GMM--random-forest comparison, and we identify this as a limitation rather than claim a separation the data do not support.

		\begin{table}[htbp]
			\begin{center}
				\caption{Comparison of DDEL-GMM with classic non-clustering ensemble methods on the HAR dataset ($10$-fold CV, $157$-PC representation). Best value in each column in bold; rank $1$ is best. $p$ is the one-sided Wilcoxon signed-rank test of DDEL-GMM against each baseline on paired per-fold F-scores ($n=10$; $p=0.001$ is the attainable minimum at this sample size). Nemenyi critical difference at $\alpha=0.05$ is 2.38 mean ranks: the difference between DDEL-GMM and random forests does not exceed it.}
				...
				\begin{tabular}{|l|c|c|c|c|c|c|}
					...
					\hline
					Single LR & 0.941 & 0.943 & 0.981 & 5.5 & 0.001 & \textbf{6.8} \\
					\hline
				\end{tabular}
			\end{center}
		\end{table}
		...
		\begin{figure*}[!t]
			...
			\caption{DDEL-GMM against classic non-clustering ensemble methods on the HAR dataset. (a)~Mean macro-$F_1$ with error bars of $\pm 1$ standard deviation over the $10$ cross-validation folds; annotations give the one-sided Wilcoxon signed-rank $p$-value against each baseline, and the header reports the Friedman omnibus test and the Nemenyi critical difference. The DDEL-GMM--random-forest rank gap falls below that critical difference and is marked not significant. (b)~Accuracy--cost trade-off: mean macro-$F_1$ against total $10$-fold training time.}
