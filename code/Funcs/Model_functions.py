from torch._VF import nan_to_num
...
class Ensemble:
    ...
    def predict_sample(self, X, Y):
        ...
        if self.normalization:
            ...
            # new
            # if any(numpy.isnan(arr).any() for arr in X_pred_for_clusters):
            #     X_pred_for_clusters_with_nan = X_pred_for_clusters.copy()
            #     X_pred_for_clusters = [numpy.nan_to_num(x, nan=0) for x in X_pred_for_clusters]
            # else:
            #     X_pred_for_clusters_with_nan=None
            #

            if any(numpy.isnan(arr).any() for arr in X_pred_for_clusters):
                X_pred_for_clusters_with_nan, X_pred_for_clusters = X_pred_for_clusters.copy(), [numpy.nan_to_num(x, nan=0) for x in X_pred_for_clusters.copy()]
            else:
                X_pred_for_clusters_with_nan = None

            # if pool classifre for each cluster
            # X_pred_for_clusters = list()
            # dissagreement_between_classifiers = list()
            # for i in range(len(self.Best_models_clusters)):
            #     X_pred_for_cluster = [model.predict_proba(X) for model in self.Best_models_clusters[i]]
            #     c = 0
            #     for l in self.classes:
            #         for o in range(len(self.cluster_centers)):
            #             class_i = self.Best_models_clusters[i][o].classes_
            #             if l not in class_i:
            #                 X_pred_for_cluster[o] = np.insert(X_pred_for_cluster[o], c, values=0, axis=1)
            #         c += 1
            #
            #     dissagreement_between_classifiers_per_cluster = np.mean(
            #         [np.corrcoef(X_pred_for_cluster[0][i], X_pred_for_cluster[1][i])[0, 1] for i in
            #          range(X_pred_for_cluster[0].shape[0])]
            #     )
            #     X_pred_for_clusters.append(X_pred_for_cluster)
            #     dissagreement_between_classifiers.append(dissagreement_between_classifiers_per_cluster)
        ...
        # if self.avge:
        #     # avge based model
        #     final_X_array_avge = np.mean([X_pred_for_clusters], axis=1).reshape(-1, len(self.classes))
        #     finall_X_tensr_avge = torch.tensor(final_X_array_avge)
        # else:
        #     final_X_array_avge = None
        # # ---------------------------------------------------------
        ...
        if self.maxe:
            ...
            # final_X_array_maxe = [max([(X_pred_for_clusters[i][k][argmax(X_pred_for_clusters[i][k])],
            #                             argmax(X_pred_for_clusters[i][k])) for i in
            #                            range(len(self.cluster_centers))])[1] for k
            #                       in range(np.array(X_pred_for_clusters).shape[1])]
            ...
            # finall_X_tensr_maxe = torch.tensor(final_X_array_maxe)
        ...
        # if self.diste:
        #     # weight of test samples belong to each cluster
        #     distances_Xtest_filtered = list()
        #     for i in range(X.shape[0]):
        #         distances_xtest = [np.linalg.norm(np.abs(self.cluster_centers[x] - X[i]))
        #                            for x in range(len(self.cluster_centers))]
        #         distances_xtest_filtered = [
        #             # distances_xtest[j] if distances_xtest[j] <= 2 * self.min_distances[j].iloc[-1].values[0]
        #             distances_xtest[j] if distances_xtest[j] <= 2 * self.min_distances[j][-1]
        #             else math.inf for j in range(len(self.cluster_centers))]

        #         if np.all(np.isinf(distances_xtest_filtered)):
        #             min_value = min(distances_xtest)
        #             min_index = distances_xtest.index(min_value)
        #             distances_xtest_filtered[min_index] = min_value

        #         distances_Xtest_filtered.append(distances_xtest_filtered)
        #     # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        #     # distance weights
        #     weights_Xtest_to_clusters = list()
        #     for i in range(len(X)):
        #         inv_distances_Xtest_filtered = np.array([(1 / (x + 1e-20)) for x in distances_Xtest_filtered[i]])
        #         wi = np.array(inv_distances_Xtest_filtered) / (inv_distances_Xtest_filtered.sum() + 1e-20)
        #         weights_Xtest_to_clusters.append(wi)
        #     weights_Xtest_to_clusters = np.array(weights_Xtest_to_clusters)
        #     # --

        #     weights_Xtest_to_clusters_tensor = [
        #         np.tile(weights_Xtest_to_clusters[:, i].reshape(-1, 1), (1, len(self.classes)))
        #         for i in range(len(self.cluster_centers))]

        #     final_X_diste = sum((np.array(weights_Xtest_to_clusters_tensor) * np.array(X_pred_for_clusters)))

        #     final_X_array_diste = np.array(final_X_diste)
        #     finall_X_tensr_diste = torch.tensor(final_X_array_diste)
        ...
        #     cluster_assignments_proba = [
        #         np.tile(cluster_assignments_proba[:, i].reshape(-1, 1), (1, len(self.classes)))
        #         for i in range(len(self.cluster_centers))]

        #     final_X_densitye = sum((np.array(cluster_assignments_proba) * np.array(X_pred_for_clusters)))
        #     final_X_array_densitye = np.array(final_X_densitye)
        #     finall_X_tensr_densitye = torch.tensor(final_X_array_densitye)
        # else:
        ...
        if self.acce:
            ...
            for i in range(X.shape[0]):
                ...
                val_models_xtest_filtered = [
                    weights_val_models[j] if distances_xtest[j] <= 2 * self.min_distances[j].iloc[-1].values[0]
        ...
        Results = {'models_outputs': models_outputs,
                   'oracle_acc': oracle_acc,
                    #  'distances_xtest_filtered': distances_Xtest_filtered,
...
class MainModel(nn.Module):
    ...
    def forward(self, x, y):
        ...
        self.EModel.Best_models_clusters = [x.set_params(loss='log_loss') if isinstance(x, SGDClassifier) else x for x in self.EModel.Best_models_clusters]
        #
        # cpu
        #ensemble_results = self.EModel.predict_sample(x.numpy(), y)
        # gpu
        ...
        for emodel in use_ensemble_list:
            if self.use_ensemble[emodel]:
                ...
                # print('++++++++++++++++ emodel_output: {} \n {} ++++++++++++++++++'.format(emodel, emodel_output))
                # print('++++++++++++++++ emodel_output: {} ++++++++++++++++++'.format(emodel))
...
def train(model, optimizer, criterion, train_loader, use_ensemble):
    ...
    for i, (inputs, labels) in enumerate(train_loader, 0):
        if inputs.shape[0] < 2:
            break
        optimizer.zero_grad()
        forward_train_outputs = model(inputs)
        ...
        # loss_emodels_train = {str(sub_model): criterion(forward_train_outputs['ensemble_outputs'][sub_model], labels)
        #                       for sub_model in list(use_ensemble.keys())}
        # final_train_main_model_loss = torch.sum(forward_train_outputs['main_gate'] *
        #                                          torch.tensor(list(loss_emodels_train.values())), dim=1)
        # final_train_main_model_loss_scaler = torch.sum(final_train_main_model_loss) / len(
        #     final_train_main_model_loss)
...
def validate(model, criterion, val_loader, use_ensemble):
    ...
    with torch.no_grad():
        for inputs, labels in val_loader:
            forward_eval_outputs = model(inputs)
            ...
            # loss_emodels_eval = {str(sub_model): criterion(forward_eval_outputs['ensemble_outputs'][sub_model],
            # labels) for sub_model in list(use_ensemble.keys())} finall_eval_main_model_loss = torch.sum(
            # forward_eval_outputs['main_gate'] * torch.tensor(list(loss_emodels_eval.values())),
            # dim=1) final_eval_main_model_loss_scaler =\ torch.sum(finall_eval_main_model_loss) / len(
            # finall_eval_main_model_loss)
...
def train_main_model(model, optimizer, criterion_main_model, n_epoch, preprocessing_res, use_ensemble):
    ...
    for epoch in tqdm(range(n_epoch), position=0, leave=True, unit='epoch', colour='green'):
        ...
        print(f"Epoch {epoch + 1:02d}: Training loss = {train_loss:.3f}, Validation loss = {val_loss:.3f}")
...
def f1_tuned_model(cs, csy, model, model_name, param_grid, normalization):
    if normalization:
        ...
        grid = GridSearchCV(estimator=pipe, param_grid=param_grid, scoring='f1_macro', cv=4, return_train_score=True, n_jobs=-1)
    else:
        grid = GridSearchCV(estimator=model, param_grid=param_grid, scoring='f1_macro', cv=4, return_train_score=True, n_jobs=-1)
    grid.fit(cs, csy)
...
def grid_train_base_models(models, cluster_samples, cluster_samples_lables, model_param_grids, normalization):
    best_param_models_samples = list()
    for n_c in tqdm(range(len(cluster_samples)), position=0, leave=True, unit='epoch', colour='green'):
